...
func InitializeLCD(nValidators int, initAddrs []sdk.AccAddress, minting bool, portExt ...string) (
	...
	db := dbm.NewMemDB()
	gapp := app.NewMdukeyApp(logger, db, nil, true, 0, baseapp.SetPruning(store.PruneNothing))
	cdc = app.MakeCodec()
...
func startTM(
	...
	privVal tmtypes.PrivValidator, app *app.MdukeyApp,
) (*nm.Node, error) {
...
// NOTE: If making updates here also update cmd/mdukey/cmd/mdukeycli/main.go
